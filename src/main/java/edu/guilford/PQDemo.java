...
public class PQDemo 
{
    public static void main( String[] args )
    {
        ...
        // objects, which is defined by the compareTo method of the FullName
        // and the sortOrder attribute of the FullName class

        // Whenever we instantiate a Collection (or Map), we should specify the class of objects 
        // that can be stored in it. If we don't inclued that, it's equivalent to 
        ...
        System.out.println("Priority queue in order:");
        while (!pq.isEmpty()) {
        ...
        while (!s.isEmpty()) {
            ...
            // then we add that element to the TreeSet
            nameSet.add(s.pop());
        }
        ...
        // By using TreeMap, we make keys sorted and easily searchable
        ...
        for (int i = 0; i < 20; i++) {
            ...
            // I f the key is already in the map, generate a new FullName object
        ...
        // Each key is associated with a list of all FullName objects with the same last name
        // Add 20 FullName objects to the Map
        for (int i = 0; i < 20; i++) {
            FullName name = new FullName();
            String key = name.getLastName();
        ...
        System.out.println("The entry for the key Jones is: " + map2.get("Jones"));
